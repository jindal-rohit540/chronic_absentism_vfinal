...
| School & Network View | `chronic_absenteeism_app.py` | Aggregate risk view — filter by Network 1–16 or governance type, top 10 schools, drill to school-level student list |
...
| `Built-in/dim_schools.csv` | 2,236 schools with SCHOOL_CODE, SCHOOL_NAME, NETWORK — joined onto predictions in Page 3 to power Network 1–16 filter; also referenced in Page 2 network selectbox |
...
## UI Decisions & Change History

### Page 2 — Student Risk Predictor

**School Network field (added 2026-06-10)**
A "School Network" selectbox was added to the Demographics section (below Gender/Race/Grade). Options: Not Specified → Network 1–16 → Charter, Options, Contract, ISP, ALOP, SAFE. This is a **contextual/display field only** — it is not a feature in the XGBoost model and does not affect the risk score.

**Dental Screening and Vision Screening removed (2026-06-10)**
Both fields were removed from the Student Health section. They are hardcoded to `"Yes"` internally so model inputs remain at their neutral default. Only `# Chronic Medical Conditions` and `Immunizations Up to Date` remain in the Student Health section.

### Page 3 — School & Network View

**Network 1–16 filter (added 2026-06-10)**
`load_predictions()` now joins `dim_schools.csv` on `SCHOOL_NAME` to attach a `NETWORK` column to every student row. The network dropdown shows: `Entire District (All CPS)` → `Network 1` through `Network 16` (numerically sorted) → then other types (Charter, Contract, ISP, Options, TASSEL) alphabetically. If a school is not found in `dim_schools`, it falls back to its `GOVERNANCE` value (0 nulls in practice across 342,018 rows). All downstream filtering — school ranking bars, student table, CSV export — uses the `NETWORK` column.
