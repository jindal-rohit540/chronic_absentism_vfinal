...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown("<div class='section-sub'>Leave on \"All Schools\" to see the full network, or pick one school to see its students.</div>", unsafe_allow_html=True)

    available_schools = ["All Schools in " + selected_network] + sorted(network_df["SCHOOL_NAME"].dropna().unique().tolist())
    sc1, sc2 = st.columns([1.5, 2.5])
    with sc1:
        selected_school = st.selectbox("School", available_schools, label_visibility="collapsed")
    ...
    <div class='section-sub'>
        Our model gives every student a likelihood score from 0–100%.
        Use this to control how many students your team follows up on.
        <b>Lower = cast a wider net. Higher = focus only on the most likely cases.</b>
    </div>
    """, unsafe_allow_html=True)

    sens_col1, sens_col2 = st.columns([2, 3])
    with sens_col1:
        sensitivity = st.radio(
            "Flagging sensitivity",
            ["Wider net — flag more students (40%+)",
             "Balanced — recommended (50%+)",
             "Focused — only the most likely (60%+)"],
            index=1,
            label_visibility="collapsed",
        )
    THRESHOLD_DISPLAY = 0.40 if "40%" in sensitivity else (0.50 if "50%" in sensitivity else 0.60)
    with sens_col2:
        pct_flagged = (view_df["risk_score"] >= THRESHOLD_DISPLAY).mean()
        st.markdown(f"""
        <div style='background:#F8FAFC; border:1px solid #E2E8F0; border-radius:10px;
            ...
            <div style='font-size:0.82rem; color:#475569; margin-top:2px;'>
                in <b>{view_label}</b> would be flagged for follow-up
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    n_urgent = int((view_df["risk_score"] >= 0.60).sum())
    n_safe   = n_total - n_atrisk
    ...
    cards = [
        ...
        (kc3, f"{n_urgent:,}",      "Need urgent attention",
         "Very high likelihood — reach out in week one",                   "#DC2626"),
        (kc4, f"{n_safe:,}",        "On track",
         "Low likelihood — standard monitoring is fine",                   "#16A34A"),
    ]
    ...
    if atrisk_df.empty:
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "Last Year Attend.": st.column_config.TextColumn("Attended Last Year", width="medium"),
            }
        )
    ...
    <div style='font-size:0.74rem; color:#94A3B8; line-height:1.8;'>
        <b>How were these students identified?</b> Each student was scored using a model trained on
        8 years of CPS records covering 529,405 students. The model looks at last year's attendance,
        health records, housing stability, and other factors to estimate the likelihood of missing
        too many days this year. Student IDs are anonymized in this view.
    </div>
    """, unsafe_allow_html=True)
